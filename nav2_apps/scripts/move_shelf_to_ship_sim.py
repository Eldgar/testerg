...
from set_costmap_parameters import set_inflation_radius, set_costmap_parameters, set_footprint

# Define destinations
destinations = {
    "pre_loading_position": [2.34, 0.2, (math.pi/2)],
    ...
    for destination in destination_sequence:
        ...
        if result == TaskResult.SUCCEEDED:
            ...
            if destination == "loading_position":
                current_obstacle_min = nav_elevator_node.get_parameter('obstacle_layer.scan.obstacle_min_range').value
                ...
                if set_costmap_parameters(nav_elevator_node, parameter_clients, raytrace_min=0.97, obstacle_min=0.97):
                    set_inflation_radius(nav_elevator_node, parameter_clients, inflation_radius=0.56)
                    ...
                    if deactivate_costmaps(costmap_lifecycle_node):
                        ...
                        if success:
                            ...
                            if activate_costmaps(costmap_lifecycle_node):
                                ...
                                current_obstacle_min = nav_elevator_node.get_parameter('obstacle_layer.scan.obstacle_min_range').value
                                current_raytrace_min = nav_elevator_node.get_parameter('obstacle_layer.scan.raytrace_min_range').value
                        ...
                        rclpy.spin_once(nav_elevator_node, timeout_sec=1.0)
                        rclpy.spin_once(costmap_lifecycle_node, timeout_sec=1.0)
                    else:
